import { searchFuzzy } from './fuzzy.js';
...
export interface PromptCommandRouter {
...
export function createPromptCommandRouter(
  ...
  const completeCommands = (query: string, maxResults: number) =>
    searchFuzzy(commands, normalizeCommandToken(query), [
      { value: (command) => command.command },
      { value: (command) => command.description, weight: 0.5 },
      { value: (command) => command.aliases?.join(' '), weight: 0.6 },
      { value: (command) => command.tags?.join(' '), weight: 0.6 },
    ], maxResults)
      .flatMap((result) => {
        const command = commandsById.get(result.item.id);
        if (!command) {
          return [];
        }
