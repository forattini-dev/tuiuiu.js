...
/**
 * Align text within a fixed width (ANSI-aware)
 */
...
/**
 * Truncate text to a visual width (handles wide chars)
 */
...
function truncateText(text: string, maxLength: number): string {
  ...
  if (textWidth <= maxLength) return text;
  if (maxLength <= 3) return truncateToWidth(text, maxLength);
  return truncateToWidth(text, maxLength - 1) + '…';
}
