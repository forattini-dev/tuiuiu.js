...
export function renderSelect<T = any>(
  ...
  if (searchable && (isSearching || searchQuery)) {
    rows.push(
      Box(
        ...
        Text({ color: 'cyan' }, '🔍 '),
        searchQuery
          ? Text({ color: 'white' }, searchQuery)
          : Text({ color: 'gray', dim: true }, searchPlaceholder),
        isSearching ? Text({ backgroundColor: 'white', color: 'black' }, ' ') : Text({}, '')
      )
    );
  }
    ...
    if (item.group && item.group !== lastGroup) {
      ...
      rows.push(
        Box(
          ...
          Text({ color: 'yellow', bold: true }, `── ${item.group} ──`)
        )
      );
    }
  ...
  if (scrollOffset > 0) {
    rows.unshift(
      Box(
        { flexDirection: 'row' },
        Text({ color: 'gray', dim: true }, '  ↑ more above')
      )
    );
  }
  if (scrollOffset + visibleItems.length < filtered.length) {
    rows.push(
      Box(
        { flexDirection: 'row' },
        Text({ color: 'gray', dim: true }, '  ↓ more below')
      )
    );
  }
  ...
  return Box({ flexDirection: 'column', onScroll: handleScroll }, ...rows);
}
...
export function Confirm(options: {
  ...
  return Box(
    ...
    Box(
      { marginBottom: 1 },
      Text({ color: 'yellow' }, '? '),
      Text({ color: 'white', bold: true }, message)
    ),
    Select({
      items,
