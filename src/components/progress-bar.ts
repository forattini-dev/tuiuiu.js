...
export function renderProgressBar(
  ...
  if (indeterminate) {
    const frame = Math.floor(Date.now() / 100) % INDETERMINATE_FRAMES.length;
    barContent = INDETERMINATE_FRAMES[frame];
  } else {
    ...
    const emptyWidth = Math.max(0, width - stringWidth(filled));
  ...
  if (showPercentage && !indeterminate) {
    infoParts.push(Text({ color: 'white' }, ` ${(progress * 100).toFixed(0)}%`));
  }

  if (showValue && options.value !== undefined && options.max !== undefined) {
    infoParts.push(Text({ color: 'gray' }, ` ${options.value}/${options.max}`));
  }

  if (showEta && !indeterminate) {
    const eta = state.getEta();
    if (isFinite(eta) && eta > 0) {
      infoParts.push(Text({ color: 'gray', dim: true }, ` ETA: ${formatTime(eta)}`));
    }
  }
  ...
  if (showSpeed) {
    ...
    if (speed > 0) {
      infoParts.push(Text({ color: 'gray', dim: true }, ` ${formatNumber(speed)}${speedUnit}`));
    }
  }
  ...
  return Box(
    ...
    Text({}, rightBorder),
    ...infoParts,
    description ? Text({ color: 'gray', dim: true }, ` ${description}`) : Text({}, '')
  );
}
...
export function ProgressBar(options: ProgressBarOptions): VNode {
  ...
  return Box(
    ...
    showPercentage && !indeterminate
      ? Text({ color: 'white' }, ` ${(progress * 100).toFixed(0)}%`)
      : Text({}, '')
  );
}
...
export function MultiProgressBar(options: {
  ...
  for (const segment of segments) {
    ...
    if (segmentWidth > 0) {
      barParts.push(Text({ color: segment.color }, '█'.repeat(segmentWidth)));
  ...
  if (emptyWidth > 0) {
    barParts.push(Text({ color: 'gray', dim: true }, '░'.repeat(emptyWidth)));
  }
      ...
      if (segment.label) {
        legendParts.push(
          Box(
            ...
            Text({ color: 'gray' }, `${segment.label}: ${segment.value}`)
          )
        );
      }
    }
